...
### Windows
No additional setup required. The plugin uses Windows Explorer via `CreateProcess` to reliably open folders.

**Note:** Windows Explorer always exits immediately when it delegates to an already-running instance. The plugin handles this correctly and reports success based on whether the process was launched, not its exit code.
...
### Windows: CMake Error (Compatibility with CMake < 3.5)
If you encounter the following error when building for Windows:
`Compatibility with CMake < 3.5 has been removed from CMake.`

This is typically caused by the `googletest` dependency in newer versions of CMake (3.27+). You can resolve this by adding the following line to your `windows/CMakeLists.txt` file, right after `cmake_minimum_required`:

```cmake
set(CMAKE_POLICY_DEFAULT_CMP0148 OLD)
```

Or, if you are not running tests, you can disable them in the `example/windows/CMakeLists.txt`:
```cmake
set(include_open_folder_tests FALSE)
```
